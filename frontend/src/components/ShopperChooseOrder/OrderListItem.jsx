                    ...
                    >
                        ...
                        <Box>
                            <Box display={"grid"} gridTemplateColumns={"max-content max-content"} gap={"10px"}>
                                <Typography sx={{ mb: 2 }} variant={"h6"} fontWeight="bold">{order?.createdBy?.firstName} {order?.createdBy?.lastName}</Typography>
                                <Rating readOnly defaultValue={stars || null} precision={0.5} />
                            </Box>
                            <Box display={"grid"} gridTemplateColumns={"min-content auto"} gap={"10px"} >
                                <ShoppingCartOutlinedIcon />
                                {(order?.groceryShop) ?
                                    <Typography variant={"body1"}><b>Shop:</b> {order?.groceryShop?.name}, {order?.groceryShop?.street}, {order?.groceryShop?.city}</Typography>
                                    :
                                    <Typography variant={"body1"}><b>Shop:</b> No Shop Specified</Typography>
                                }

                                <LocationOnOutlinedIcon />
                                <Typography variant={"body1"}><b>Destination:</b> {order?.destination?.street}, {order?.destination?.city}</Typography>
                                <ListOutlinedIcon />
